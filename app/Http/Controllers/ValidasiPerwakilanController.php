    ...
    public function show($id)
    {
        ...
        if (Auth::user()->role != 'Pemerintah Daerah') {
            if (Auth::user()->kd_pwk == 'PW12') {
                ...
                $validasiPemdas = DataPemda::where('data_baseline.kd_pwk', $id)->join('data_baseline' , 'data_pemda.id' , '=' , 'data_baseline.id')->orderBy('data_baseline.id')->get();
            }
        } else {
            $validasiPemdas = DataPemda::where('data_baseline.kd_pwk', $id)->where('data_baseline.nm_pemda', Auth::user()->nm_pemda)->join('data_baseline' , 'data_pemda.id' , '=' , 'data_baseline.id')->orderBy('data_baseline.id')->get();
        }

        $rincianBansos = UjiAktBansos::where('data_baseline.kd_pwk', $id)->whereBetween('kd_rek', [15, 32])->join('data_baseline' , 'mon_bansos.kd_pemda' , '=' , 'data_baseline.id')->get();
