<div class="table-responsive">
    <table class="table table-bordered text-center" id="validasiPerwakilans-table">
        ...
        @foreach($validasiPerwakilans as $validasiPerwakilan)
            ...
            <td>
                <span class="badge text-xs {{ $validasiPerwakilan->sum_a_5_1_06 != $rincianBansos->where('kd_pwk' , $validasiPerwakilan->kd_pwk)->sum('uji_anggaran') || $rincianBansos->where('kd_pwk' , $validasiPerwakilan->kd_pwk)->count() == 0 || $rincianBansos->where('kd_pwk' , $validasiPerwakilan->kd_pwk)->groupBy('nm_pemda')->count() != $validasiPerwakilan->where('kd_pwk' , $validasiPerwakilan->kd_pwk)->count() ? 'bg-danger' : 'bg-primary' }}">@persen($validasiPerwakilan->sum_a_5_1_06 > 0 ? $rincianBansos->where('kd_pwk' , $validasiPerwakilan->kd_pwk)->groupBy('nm_pemda')->count() / $validasiPerwakilan->where('kd_pwk' , $validasiPerwakilan->kd_pwk)->count() * $rincianBansos->where('kd_pwk' , $validasiPerwakilan->kd_pwk)->sum('uji_anggaran') / $validasiPerwakilan->sum_a_5_1_06 * 100 : 0)%</span></td>
                <td width="220">
                    <div class='btn-group'>
