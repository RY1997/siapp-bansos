<div class="table-responsive">
    <table class="table table-bordered text-center" id="validasiPerwakilans-table">
        ...
        @foreach($validasiPerwakilans as $validasiPerwakilan)
            ...
            <td>@rupiah($rincianBansos->where('kd_pwk' , $validasiPerwakilan->kd_pwk)->count() > 0 ? $rincianBansos->where('kd_pwk' , $validasiPerwakilan->kd_pwk)->pluck('sum_uji_anggaran')->first() : 0)</td>
            <td>@rupiah($rincianBansos->where('kd_pwk' , $validasiPerwakilan->kd_pwk)->count() > 0 ? $rincianBansos->where('kd_pwk' , $validasiPerwakilan->kd_pwk)->pluck('sum_uji_realisasi')->first() : 0)</td>
